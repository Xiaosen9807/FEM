...
def LobattoDerivative(xi, Order):
    ...
    print(dLo[:, 0:Order+1].shape)
    print(dLo[:, Order:Order+1])
    return dLo[:, Order-1:Order+1]
...
def MassAndStiffness_1D(iElem, Order, Coord, Element):
  ...
  GaussPoints, GaussWeights = np.polynomial.legendre.leggauss(int(5))

  NrOfGaussPoints = GaussWeights.size
  print(NrOfGaussPoints)
